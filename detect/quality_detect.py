...
class QualityDetect(DetectBase):
	...
	def __init__(self, image_filter, image_url, image_source='weibo', image_cv=None):
		# image_url 内网照片url
	...
	def get_image_faceinfo(self):
		...
		logger.info('face number: {}, face_normal_positions: {}, positions: {}, landmarks: {}'.
					format(num, face_normal_positions, positions, landmarks))
	...
	def tag_base_info(self):
		...
		return base_tags, {'image-blur': blur, 'image-resolution': int(resolution), 'image_source': self.image_source,
	...
	def tag_faces_info(self, max_face_num=3):
		...
		new_face_normal_positions = deepcopy(face_normal_positions)
		# np array
		...
		for index, pos in enumerate(face_positions):
			...
			if face_h > 0.05:
				...
				# 更新人脸信息去除掉忽略掉的人脸，为下面的肤色识别做准备
				# 这里做了个假设，landmark序要和fface_normal_positions是对应的，这样才能删除不需要的人脸信息
				np.delete(face_landmarks, index, axis=1)
				# mutable 数据结构，不用更新face_info了

				new_face_normal_positions.remove(face_normal_positions[index])
		...
		return True, msg, new_face_normal_positions, new_face_num
	...
	def test_img_clothes(self):
		...
		for clothes_detect_dict in clothes_detect_results:
				...
				xmin, ymin, xmax, ymax = \
					max(0, float(clothes_detect_dict['xmin'])), \
					max(0, float(clothes_detect_dict['ymin'])), \
					min(1, float(clothes_detect_dict['xmax'])), \
					min(1, float(clothes_detect_dict['ymax']))
		...
		if not (clothes_detect_results and level_one_labels):  # 无可用服装
	...
	def filter_dedup_image(self, url):
		...
		# [url, feature_hash]
		hash_result: List[str, int] = self.image_worker.filter_hash_image(url, self.data)
