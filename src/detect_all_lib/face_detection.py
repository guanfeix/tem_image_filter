...
def alignment(src_img, src_pts):
	of = 2
	ref_pts = [[30.2946 + of, 51.6963 + of], [65.5318 + of, 51.5014 + of],[48.0252 + of, 71.7366 + of], [33.5493 + of, 92.3655 + of], [62.7299 + of, 92.2041 + of]]
	crop_size = (96, 112)
...
class mtcnnNet():
	...
	def create_mtcnn(self, sess):
		if not self.model_path:
			self.model_path, _ = os.path.split(os.path.realpath(__file__))

		with tf.variable_scope('pnet'):
			data = tf.placeholder(tf.float32, (None, None, None, 3), 'input')
			pnet = mtcnnNet.PNet({'data': data})
			pnet.load(os.path.join(self.model_path, 'det1.npy'), sess)
		with tf.variable_scope('rnet'):
			data = tf.placeholder(tf.float32, (None, 24, 24, 3), 'input')
			rnet = mtcnnNet.RNet({'data': data})
			rnet.load(os.path.join(self.model_path, 'det2.npy'), sess)
		with tf.variable_scope('onet'):
			data = tf.placeholder(tf.float32, (None, 48, 48, 3), 'input')
			onet = mtcnnNet.ONet({'data': data})
			onet.load(os.path.join(self.model_path, 'det3.npy'), sess)
	...
	def detect_face(self, img, minsize, pnet, rnet, onet, threshold, factor):
		factor_count = 0
		...
		for j in range(len(scales)):
			...
			# inter-scale nms
			pick = self.nms(boxes.copy(), 0.7, 'Union')
...
class faceDetect():
	...
	人脸检测类,用于人脸检测
	"""
	def __init__(self, model_path = 'model_path', minsize=50, threshold=[0.6, 0.7, 0.9], factor=0.709):
		...
		self.factor = factor
		self.mtcnn_net = mtcnnNet(self.model_path)
		with self.sess.as_default():
			self.pnet, self.rnet, self.onet = self.mtcnn_net.create_mtcnn(self.sess)
